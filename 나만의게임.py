...
def get_bg():
    with open("img/duck_bg.jpg", "rb") as img_file:
...
if st.session_state.room < len(rooms):
    ...
    st.markdown(f"<div class='title'>🎉 게임 종료! 결과 보기! 🎉</div>", unsafe_allow_html=True)
    for i, (choice, result) in enumerate(st.session_state.history):
        st.markdown(f"**STEP {i+1}** - *{choice}* \n➡️ {result}")
