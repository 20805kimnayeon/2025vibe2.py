...
# 🎨 테마 선택
theme = st.sidebar.selectbox("🎨 테마를 선택하세요!", ["🎀 공주", "🧙 마법사", "🏴‍☠️ 해적", "🪐 우주"])
...
if theme == "🎀 공주":
    ...
    set_background("img/pirate.png")
    emoji_set = ["🏴‍☠️", "🪙", "⚓", "🦜", "🗺️"]
    primary_color = "#f4a460"
elif theme == "🪐 우주":
    set_background("img/space.png")
    emoji_set = ["🚀", "🪐", "🌠", "👽", "⭐"]
    primary_color = "#9bcaff"
    ...
    with col1:
        if st.button("✅ 맞혀보기"):
            ...
            if guess == st.session_state.target:
                ...
                st.success(f"{random.choice(emoji_set)} 정답이에요! 축하해요! {random.choice(emoji_set)}")
                st.balloons()
                st.session_state.game_over = True
            elif guess < st.session_state.target:
                play_audio("audio/wrong.mp3")
                st.warning(f"{random.choice(emoji_set)} 너무 작아요!")
            else:
                play_audio("audio/wrong.mp3")
                st.warning(f"{random.choice(emoji_set)} 너무 커요!")

            if st.session_state.tries >= max_tries and not st.session_state.game_over:
                st.error(f"💣 기회를 모두 소진했어요! 정답은 **{st.session_state.target}** 였어요!")
                st.session_state.game_over = True
